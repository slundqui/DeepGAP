...
class SupVidMLP2_kitti(TFObj):
    ...
    def getLoadVars(self):
        ...
        if(self.resLoad):
            ...
            v = [var for var in v if (("V1_W" in var.name) or ("hidden_bias" in var.name) or ("conv1_w" in var.name) or ("conv1_b" in var.name) or ("class_weight" in var.name) or ("class_bias" in var.name)) and ("Adam" not in var.name)]
        else:
