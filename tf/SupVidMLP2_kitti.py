...
class SupVidMLP2_kitti(TFObj):
    def makeDirs(self):
        super(SupVidMLP2_kitti, self).makeDirs()
    ...
    def defineVars(self):
        ...
        self.conv2_w = weight_variable([1, 1, self.numFeatures, self.numFeatures], "conv2_w", 1e-6)
        self.conv2_b = weight_variable([self.numFeatures], "conv2_b", 1e-6)
    ...
    def buildModel(self, inputShape):
        ...
        with tf.device(self.device):
            ...
            with tf.name_scope("Loss"):
                ...
                self.weightRegLoss = tf.reduce_sum(tf.square(self.class_weight)) + tf.reduce_sum(tf.square(self.conv2_w)) + tf.reduce_sum(tf.square(self.conv1_w)) + tf.reduce_sum(tf.square(self.h_weight))
    ...
    def getLoadVars(self):
        ...
        if(self.resLoad):
            ...
            v = [var for var in v if (("hidden_weight" in var.name) or ("hidden_bias" in var.name) or ("conv1_w" in var.name) or ("conv1_b" in var.name) or ("class_weight" in var.name) or ("class_bias" in var.name)) and ("Adam" not in var.name)]
        else:
